...
typedef enum ListResult {
	...
	LIST_NULL_ELEMENT_ERROR,			/**< Item or address to item are NULL			*/
	LIST_UNDERFLOW_ERROR				/**< Pop attempt on empty list					*/
} ListResult;
 ...
 *  @details if successful, stores a pointer to the removed item in _item
 *  time complexity O(1).
 *
 * @params _list : A previously created List ADT returned via ListCreate
 ...
 * @retval LIST_SUCCESS on success
 * @retval LIST_UNINITIALIZED_ERROR if the list is not initialized
 * @retval LIST_NULL_ELEMENT_ERROR if address to item is NULL
 ...
 *
 * @params _list : A previously created List ADT returned via ListCreate
 * @returns list's number of nodes (not including sentinels) 
 *
 * @warning : if _list is NULL, 0 is returned.
 */
