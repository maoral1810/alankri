...
typedef enum VectorResult VectorResult;
/**  
 ...
 * @retval VECTOR_ALLOCATION_ERROR 	- if reallocation failed
 * @retval VECTOR_OVERFLOW 		 	- if vector increment size = 0
 * @retval VECTOR_UNITIALIZED_ERROR - if vector is NULL
 */
VectorResult VectorAppend(Vector* _vector, void* _item);

/**  
 * @brief Delete an Element from the back of the Vector. time complexity: O(1)
 * @param[in] _vector - Vector to delete integer from.
 * @param[out] _pValue - pointer to variable that will receive deleted item value
 * @return success or error code 
 * @retval VECTOR_SUCCESS on success 
 * @retval VECTOR_UNDERFLOW			- remove from an empty vector
 * @retval VECTOR_ALLOCATION_ERROR  - if reduse vector failed
 * @retval VECTOR_UNITIALIZED_ERROR - if vector is NULL 
 ...
 * @return success or error code 
 * @retval VECTOR_SUCCESS on success 
 * @retval VECTOR_INDEX_OUT_OF_BOUNDS_ERROR - if index is invalid 
 * @retval VECTOR_UNITIALIZED_ERROR 		- if vector is NULL  
